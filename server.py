...
def book(competition, club):
    ...
    if selected_club and selected_competition:
        ...
        competition_date = datetime.strptime(selected_competition['date'], "%Y-%m-%d %H:%M:%S")
        ...
        # limited by the 12-place rule, the club's available points, and remaining competition spots
        max_places = min(12, int(selected_club['points']), int(selected_competition['numberOfPlaces']))
        return render_template('booking.html', club=selected_club, competition=selected_competition, max_places=max_places)
    else:
...
def purchasePlaces():
    ...
    selected_competition['numberOfPlaces'] = int(selected_competition['numberOfPlaces']) - places_requested
